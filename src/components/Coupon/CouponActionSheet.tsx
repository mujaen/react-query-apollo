...
const CouponActionSheet: React.FC<CouponActionSheetProps> = ({
  ...
  const makeOnClickCard = useCallback(
    (coupon: CouponData) => () => {
      const filteredSelectCoupons = currentSelectCoupon?.filter(select => select.id !== coupon.id);
      const selectedCoupon = currentSelectCoupon?.filter(select => select.id === coupon.id);

      setCurrentSelectCoupon(
        selectedCoupon && selectedCoupon.length > 0 ? filteredSelectCoupons : [...currentSelectCoupon, coupon]
      )
